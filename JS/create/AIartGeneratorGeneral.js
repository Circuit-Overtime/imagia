...
window.onload = function() {
    ...
    downloadUrl = "http://192.168.40.18:3000";
    pingUrl = "http://192.168.40.18:3000";
    enhanceUrl = "http://192.168.40.18:3002"
...
async function generatingModeHandle() {
    ...
    if (enhanceSwitch) {
        ...
        document.getElementById("enhancedPrompt").innerText = await enhancePrompt();
        document.getElementById("hqlqcontainer").classList.add("hidden");
    } else {
        document.getElementById("aiEnhancementDesc").classList.add("hidden");   
        document.getElementById("privatePublicResultDesc").classList.add("hidden");
...
function expandImage(enc) {
   ...
   let encodedEnteries = enc.split("###");
    ...
    document.getElementById("upscaleImage").setAttribute("data-id", encodedEnteries[0]);
    setTimeout(() => {
...
document.getElementById("upscaleImage").addEventListener("click", (e) => {
    const upscaleImage = document.getElementById("upscaleImage").getAttribute("data-id");
    upscaleImageBlob(upscaleImage);
})
async function upscaleImageBlob(input) {
    document.getElementById("backButton").style.pointerEvents = "none";
    document.getElementById("upscaleImage").style.pointerEvents = "none";
    ...
    try {
        ...
        if (typeof input === "string" && input.startsWith("data:image")) {
            ...
            throw new Error("Invalid input type: Expected Blob or Base64 string.");
        }
        ...
        const response = await fetch("http://192.168.40.18:5000/upscale", {
            method: "POST",
            body: formData
        });
        ...
        // Update UI
        const imgElement = document.getElementById("showImage")?.querySelector("img");
        if (imgElement) imgElement.src = imageUrl;

        const downloadBox = document.getElementById("downloadBox");
        if (downloadBox) downloadBox.setAttribute("data-id", imageUrl);

        const savedMsg = document.getElementById("savedMsg");
        if (savedMsg) {
            savedMsg.classList.add("display");
            setTimeout(() => savedMsg.classList.remove("display"), 1500);
        }

        // Prevent memory leaks
        // setTimeout(() => URL.revokeObjectURL(imageUrl), 5000);
        document.querySelector(".wrapper").classList.add("hidden");
        document.getElementById("backButton").style.pointerEvents = "all";
        document.getElementById("upscaleImage").style.pointerEvents = "none";
...
function base64ToBlob(base64, mimeType = "image/png") {
    ...
    let byteNumbers = new Uint8Array(byteCharacters.length).map((_, i) => byteCharacters.charCodeAt(i));
